...
let boundRuntime: FlowRuntime | undefined;

export function bindFlowRuntime(runtime: FlowRuntime): void {
  boundRuntime = runtime;
}

function runtime(): FlowRuntime {
  if (!boundRuntime) {
    throw new Error('Flowctl runtime is not bound. Import bindFlowRuntime from this generated module in an application step bootstrap and bind the reviewed Playwright runtime adapter.');
  }
  return boundRuntime;
}
...
Given('data requirements for {string} are satisfied', async ({}, variantId: string) => runtime().prepareData(variantId));
Given('flow choice for requirement {string} is {string}', async ({}, requirementId: string, value: string) => runtime().applyFlowAssignment(requirementId, value));
Given('an actor satisfying {string} with requirement IDs {string} starts the journey', async ({}, actor: string, ids: string) => runtime().startJourney(actor, splitIds(ids)));
Given('the actor is on page ID {string} named {string}', async ({}, pageId: string, page: string) => runtime().ensurePageDisplayed(pageId, page));
When('the actor completes page ID {string} named {string}', async ({}, pageId: string, page: string) => runtime().completePage(pageId, page));
When('the actor performs action ID {string} named {string}', async ({}, actionId: string, action: string) => runtime().performAction(actionId, action));
When('a source-valid value is entered for {string} using field ID {string} on page ID {string}', async ({}, field: string, fieldId: string, pageId: string) => runtime().enterValidFieldValue(field, fieldId, pageId));
When('the value for field ID {string} satisfies constraint ID {string} of kind {string} in domain {string} with source value {string}', async ({}, fieldId: string, constraintId: string, kind: string, domain: string, sourceValue: string) => runtime().expectFieldConstraintSatisfied(fieldId, constraintId, kind, domain, sourceValue));
When('the value for field ID {string} satisfies constraint ID {string} of kind {string} in domain {string}', async ({}, fieldId: string, constraintId: string, kind: string, domain: string) => runtime().expectFieldConstraintSatisfied(fieldId, constraintId, kind, domain));
When('field ID {string} on page ID {string} violates constraint ID {string} with {string}', async ({}, fieldId: string, pageId: string, constraintId: string, value: string) => runtime().violateFieldConstraint(fieldId, pageId, constraintId, value));
When('field ID {string} on page ID {string} violates constraint ID {string}', async ({}, fieldId: string, pageId: string, constraintId: string) => runtime().violateFieldConstraint(fieldId, pageId, constraintId));
Then('{string} with operation IDs {string} should succeed', async ({}, operation: string, ids: string) => runtime().expectOperationSucceeded(operation, splitIds(ids)));
Then('page ID {string} named {string} should be displayed', async ({}, pageId: string, page: string) => runtime().expectPageDisplayed(pageId, page));
Then('field ID {string} should satisfy its active validation contract', async ({}, fieldId: string) => runtime().expectFieldSatisfiesActiveValidation(fieldId));
Then('validation should prevent the page from continuing', async ({}) => runtime().expectValidationBlocksProgress());
