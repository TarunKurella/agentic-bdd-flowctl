...
import type { ActorRequirements, BehaviorGraph, FlowFamilies, FlowVariants, PageContracts } from '../ir/model.js';
...
export async function generateBdd(store: ArtifactStore, requestedFamily?: string): Promise<string[]> {
  ...
  for (const family of families.data.families.filter((candidate) => !requestedFamily || candidate.id === requestedFamily)) {
    ...
    for (const variant of familyVariants) {
      ...
      variant.pageSequence.forEach((pageId, index) => {
        const page = pages.data.pages.find((candidate) => candidate.id === pageId);
        const actionId = variant.actionSequence[index];
        const action = behavior.data.nodes.find((node) => node.id === actionId);
        if (page?.fields.length) lines.push(`    ${index === 0 ? 'When' : 'And'} the actor completes the "${page.name}" page`);
        if (action) lines.push(`    ${page?.fields.length || index > 0 ? 'And' : 'When'} the actor performs "${action.label}"`);
      });
      ...
      lines.push('');
      stepPlan.push({ variantId: variant.id, familyId: family.id, pageSequence: variant.pageSequence, actionSequence: variant.actionSequence, evidenceRefs: variant.evidenceRefs });
    }
...
export function registerFlowctlSteps(bdd: StepRegistrar, runtime: FlowRuntime): void {
  ...
  bdd.Given('the actor is on the {string} page', async (page: string) => runtime.completePage(page));
